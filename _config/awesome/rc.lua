...
for s = 1, screen.count() do
    ...
    -- Create the wibox
    mywibox[s] = awful.wibox({ position = "top", screen = s, height = statusbarheight })
...
function update_titlebar(c)
    if should_have_titlebar(c) and not c.titlebar then
        awful.titlebar.add(c, { modkey = modkey, height = titlebarheight })
    elseif not should_have_titlebar(c) and c.titlebar then
